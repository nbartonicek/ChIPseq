...
for projectname in ${projectnames[@]}; do
        ...
        inPath="/share/ScratchGeneral/nenbar/projects/Chris/project_results/ELF5_RNAseq.star/"

	outPath="$projectDir/project_results/$projectname.RNAseq_size"
...
while [ $j -lt ${#filesTotal[@]} ]; do
	...
	dup_line="samtools fixmate $outDir$name.sorted.bam $dupDir$name.dedup.bam"
        ...
        dup_line="java -Xmx2g -jar /share/ClusterShare/software/contrib/gi/picard-tools/1.95/CollectInsertSizeMetrics.jar INPUT=$inPath$name/Aligned.sortedByCoord.out.bam HISTOGRAM_FILE=$dupDir$name.metrix.jpg OUTPUT=$dupDir$name.metrix.txt"
        #echo $dup_line

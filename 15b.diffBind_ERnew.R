...
samples<-samples[grepl("ER",samples$SampleID),]

experiment = dba(sampleSheet=samples)
experiment <- dba.count(experiment)
experiment <- dba.contrast(experiment)
#for ER, where binding levels are similar use bFullLibrarySize=F
#experiment <- dba.analyze(experiment)
experiment <- dba.analyze(experiment,bFullLibrarySize=F)

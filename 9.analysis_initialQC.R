...
imageDir=paste0(resultsDir,"/figures/macs_default/")
...
experiment = ChIPQC(samples,annotation="hg38",chromosomes="chr19",blacklist=blacklist)
...
dev.off()
pdf(paste0(imageDir,"Regi.pdf"))
plotRegi(experiment,facetBy=c("Factor","Condition"))+theme(text = element_text(size=10))
dev.off()
pdf(paste0(imageDir,"peakProfile.pdf"))
plotPeakProfile(experiment,facetBy=c("Factor","Condition"))
dev.off()
pdf(paste0(imageDir,"FractionInPeaks.pdf"))
plotFrip(experiment,facet=F)+theme(text = element_text(size=10), axis.text.x = element_text(angle=90, hjust=1))
dev.off()

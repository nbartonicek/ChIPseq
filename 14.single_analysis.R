...
#summits=cleanGRs[[sampleName]]
  ...
  cat("overlap with all promoters\n")
  cat(promoterOverlap)
  cat("\n")
  ...
  for(geneType in c("all",names(deGenesL))){
    ...
    promoterOverlap<-sum(countOverlaps(peak,promoterSubset)>0)
    cat(paste0("overlap with ",geneType," promoters\n"))
    cat(promoterOverlap)
    cat("\n")
  ...
  genome(gr.hg19) = "hg19"
  export(gr.hg19,paste0(projectDir,"/project_results/beds/ELF5_hg19.bed"))
...
Biostrings::writeXStringSet(seq, paste0(projectDir,"/project_results/fasta/ELF5_500bp.bed"))


seq = BSgenome::getSeq(BSgenome.Hsapiens.UCSC.hg38, gr1kb)
names(seq) = paste0("SEQUENCE_", seq_along(seq))
Biostrings::writeXStringSet(seq, paste0(projectDir,"/project_results/fasta/ELF5_1kb.bed"))

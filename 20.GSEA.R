...
pathway<-read.table("MDSC.txt",header=T)
pathway=pathway[,1]
pathways<-list()
pathways[["MDSC"]]<-pathway
...
#extract for all pathways
names(pathways)=c("MDSC","ER_associated_genes")
pdf(paste0(imageDir,"ERassociated_GSEA_fullplot.pdf"),width=8,height=4)

plotEnrichment(pathways[["ER_associated_genes"]],
               rank) + labs(title="ER_associated_genes")
